...
########################################################################################################################
#Note: the ereefs package creater has told me this is undergoing some updates. If the function get_ereefs_ts does not work for you during the time you are downloading it, please contact the package creator or use the weblink below in the section: r eReefs extractor tool
...
Rsquare<-R2 %>%
  arrange(Rel.Importance.GBM.NatComms) %>%
  mutate(cross = factor(cross, levels=c("pure", "hybrid"))) %>%
  ggplot(aes(x = Zoox, y = Rel.Importance.GBM.NatComms, fill=LifeStage, shape = cross, color=LifeStage))+geom_linerange(aes(ymin=Rel.Importance.GBM.NatComms-SE, ymax=Rel.Importance.GBM.NatComms+SE), width=0.2, position=position_jitterdodge(dodge.width=0.65, jitter.height=0,jitter.width=0.25))+#facet_grid(.~LifeStage)+
